...
const SubCard = ({ sub, planOpts, activeTrading, typeColors, loading, onEdit, onToggle, onDelete }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem' }}>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontWeight: 700, fontSize: '1rem', color: '#1e293b', marginBottom: '0.2rem', wordBreak: 'break-word' }}>{sub.name}</div>
            <div style={{ fontSize: '0.78rem', color: '#94a3b8', fontFamily: 'monospace' }}>{sub.packageCode}</div>
          </div>
          <div style={{ display: 'flex', gap: '0.4rem', flexShrink: 0, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            <span style={{ padding: '2px 9px', borderRadius: 10, fontSize: '0.72rem', fontWeight: 700, background: typeColors.bg, color: typeColors.color }}>{sub.serviceType || 'RA'}</span>
            <span style={{ padding: '2px 9px', borderRadius: 10, fontSize: '0.72rem', fontWeight: 700, background: sub.isActive ? '#d4edda' : '#fee2e2', color: sub.isActive ? '#166534' : '#991b1b' }}>{sub.isActive ? 'Active' : 'Inactive'}</span>
      ...
      {expanded && (
        <div style={{ padding: '0 1.25rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
          ...
          {sub.features?.length > 0 && (
            <div>
              <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.4rem' }}>Features ({sub.features.length})</div>
      ...
      {/* View more toggle + actions — always at bottom */}
      <div style={{ borderTop: '1px solid #f1f5f9', padding: '0.75rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
        <button
          onClick={() => setExpanded(e => !e)}
          style={{ background: 'none', border: '1px solid #e2e8f0', borderRadius: 8, padding: '0.4rem 0', fontSize: '0.8rem', color: '#64748b', cursor: 'pointer', fontWeight: 600, width: '100%' }}
        >
          {expanded ? '▲ View Less' : '▼ View More'}
        </button>
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          <button className="admin-edit-btn" onClick={onEdit} disabled={loading} style={{ flex: 1 }}>Edit</button>
          <button className="admin-toggle-btn" onClick={onToggle} disabled={loading} style={{ flex: 1 }}>{sub.isActive ? 'Deactivate' : 'Activate'}</button>
          <button className="admin-delete-btn" onClick={onDelete} disabled={loading} style={{ flex: 1 }}>Delete</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="admin-section">
      ...
      {subscriptions.length === 0 && !loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '1.25rem', alignItems: 'start' }}>
          {subscriptions.map((sub) => {
            ...
            return (
              <SubCard
                ...
                onToggle={() => handleToggleStatus(sub._id)}
